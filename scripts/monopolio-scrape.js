...
function construirWebhookTargets(webUrl) {
    ...
    try {
        ...
        if (path.endsWith('/api/monopolio/webhook-github')) {
            targets.push(withProtocol);
        } else if (path === '/api') {
            targets.push(`${origin}/api/monopolio/webhook-github`);
        } else {
            targets.push(`${withProtocol}/api/monopolio/webhook-github`);
            targets.push(`${origin}/api/monopolio/webhook-github`);
        }
    } catch (e) {
        targets.push(`${withProtocol}/api/monopolio/webhook-github`);
...
async function run() {
    ...
    try {
        ...
        console.log('[MONOPOLIO SCRAPER] Probando API pública de Vinted...');
        const porApi = await extraerProductosPorApiVinted(url);
        if (porApi.length > 0) {
            productosExtraidos.push(...porApi);
            console.log(`[MONOPOLIO SCRAPER] API de Vinted devolvió ${porApi.length} productos.`);
        }

        // Último fallback: navegador real con Playwright para páginas anti-bot
        if (productosExtraidos.length === 0) {
            console.log('[MONOPOLIO SCRAPER] API devolvió 0 productos. Probando extracción con navegador real (Playwright)...');
            const porNavegador = await extraerProductosConPlaywright(url);
            if (porNavegador.length > 0) {
                productosExtraidos.push(...porNavegador);
                console.log(`[MONOPOLIO SCRAPER] Playwright devolvió ${porNavegador.length} productos.`);
            }
        }
